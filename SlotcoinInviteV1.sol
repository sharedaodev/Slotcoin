...
contract Slotcoin is Context, IERC20, Ownable {
    ...
    string private _name = "Slotcoin";
    string private _symbol = "SLOT";
    ...
    ISlotcoinInviteV1 public immutable invite;//invite contract
    uint256 public  _invite1Fee = 12;//invite1£¬12¡ë
    uint256 private _previousInvite1Fee = _invite1Fee;
    uint256 public  _invite2Fee = 8;//invite2£¬8¡ë
    uint256 private _previousInvite2Fee = _invite2Fee;
    address public _inviteRoot;


    address public fomoAdmin;
    address public _lastBuyAddress;
    uint256 public openTimeFoMo;
    uint256 private _minFoMoBalance = 1 * 10**6 * 10**9;
    ...
    address public router = 0xED7d5F38C79115ca12fe6C0041abb22F0A06C300;
    ...
    constructor (ISlotcoinInviteV1 _invite,address inviteRoot) {
         invite = _invite;
        _inviteRoot = inviteRoot;

        _rOwned[owner()] = _rTotal;

		addAddress(owner());

        IMdexRouter _uniswapV2Router = IMdexRouter(router);
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        _takeLotto(data.tLotto);
		_takeDev(data.tDev);

		(address upper1,address upper2) = invite.inviteUpper2(recipient);
		address invite1Upper = (upper1 == address(0))?_inviteRoot:upper1;
		address invite2Upper = (upper2 == address(0))?_inviteRoot:upper2;
		_takeInvite(data.tInvite1Fee,invite1Upper);
		_takeInvite(data.tInvite2Fee,invite2Upper);
		_takeFomo(data.tFomo);
        _takeBurn(data.tBurn);
    ...
    function setInvite1FeePercent(uint256 invite1Fee) external onlyOwner() {
        _invite1Fee = invite1Fee;
    }

    function setInvite2FeePercent(uint256 invite2Fee) external onlyOwner() {
        _invite2Fee = invite2Fee;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
		...
		uint256 tInvite1Fee = calculateInvite1Fee(tAmount);
		uint256 tInvite2Fee = calculateInvite2Fee(tAmount);
		uint256 tFomo = calculateFomoFee(tAmount);
        uint256 tBurn = calculateBurnFee(tAmount);

	    uint256 tT = tFee.add(tLiquidity).add(tLotto).add(tDev);
	    tT = tT.add(tInvite1Fee).add(tInvite2Fee).add(tFomo).add(tBurn);
        uint256 tTransferAmount = tAmount.sub(tT);
    
        //uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tLotto).sub(tDev);
        return (tTransferAmount, TData(0, tFee, tLiquidity, tLotto, tDev, 0,tInvite1Fee,tInvite2Fee,tFomo,tBurn));
    }
    ...
    function _getRValues( TData memory _data) private pure returns (uint256, uint256, uint256) {
		...
		uint256 rInvite1Fee = _data.tInvite1Fee.mul(_data.currentRate);
		uint256 rInvite2Fee = _data.tInvite2Fee.mul(_data.currentRate);
        uint256 rFomo = _data.tFomo.mul(_data.currentRate);
        uint256 rBurn = _data.tBurn.mul(_data.currentRate);

		uint256 rT = rFee.add(rLiquidity).add(rLotto).add(rDev);
		rT = rT.add(rInvite1Fee).add(rInvite2Fee).add(rFomo).add(rBurn);
    ...
    function _takeInvite(uint256 tInviteFee,address upper) private {
        uint256 currentRate =  _getRate();
        uint256 rInviteFee = tInviteFee.mul(currentRate);
        _rOwned[upper] = _rOwned[upper].add(rInviteFee);
        if(_isExcluded[upper])
             _tOwned[upper] = _tOwned[upper].add(tInviteFee);
    }
    ...
    function _takeFomo(uint256 tFomo) private {
        ...
        if(_isExcluded[fomoAdmin])
             _tOwned[fomoAdmin] = _tOwned[fomoAdmin].add(tFomo);
    }
    ...
    function calculateInvite1Fee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_invite1Fee).div(
            10**3
        );
    }

    function calculateInvite2Fee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_invite2Fee).div(
            10**3
        );
    }
    ...
    function removeAllFee() private {
        ...
        _previousInvite1Fee = _invite1Fee;
        _previousInvite2Fee = _invite2Fee;
    ...
    function restoreAllFee() private {
        ...
        _invite1Fee = _previousInvite1Fee;
        _invite2Fee = _previousInvite2Fee;
